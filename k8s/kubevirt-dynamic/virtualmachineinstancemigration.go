...
func (c *Client) CreateVirtualMachineInstanceMigration(
	...
	result, err := c.client.Resource(migrationResource).Namespace(vmiNamespace).Create(migration, metav1.CreateOptions{})
	if err != nil {
		return nil, err
	}
	// // get name
	// migrationName, found, err := unstructured.NestedString(result.Object, "metadata", "name")
	// if err != nil || !found {
	// 	return nil, fmt.Errorf("failed to get 'name' of the created vmi migration: %w", err)
	// }
	// return c.GetVirtualMachineInstanceMigration(vmiNamespace, migrationName)
...
func (c *Client) GetVirtualMachineInstanceMigration(
	...
	migration, err := c.client.Resource(migrationResource).Namespace(namespace).Get(name, metav1.GetOptions{})
	if err != nil {
		return nil, err
	}
