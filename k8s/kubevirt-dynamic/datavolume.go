...
func (c *Client) GetDataVolume(
	...
	dvRaw, err := c.client.Resource(dvResource).Namespace(namespace).Get(name, metav1.GetOptions{})
	if err != nil {
		return nil, err
	}
...
func (c *Client) ListDataVolumes(
	...
	result, err := c.client.Resource(dvResource).Namespace(namespace).List(opts)
	if err != nil {
		return nil, err
	}
	var dvs []*DataVolume
