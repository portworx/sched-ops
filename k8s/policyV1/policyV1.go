...
func New(client policyv1client.PolicyV1Interface) *Client {
	return &Client{
		policy: client,
	}
}
...
func NewForConfig(c *rest.Config) (*Client, error) {
	policy, err := policyv1client.NewForConfig(c)
	if err != nil {
		return nil, err
	}
...
type Client struct {
	config *rest.Config
	policy policyv1client.PolicyV1Interface
}
...
func (c *Client) loadClient() error {
	...
	c.policy, err = policyv1client.NewForConfig(c.config)
	if err != nil {
		return err
	}
