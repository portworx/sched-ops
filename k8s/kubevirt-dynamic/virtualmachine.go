...
type VirtualMachineOps interface {
	...
	GetVirtualMachine(namespace, name string) (*VirtualMachine, error)
	// ListVirtualMachines retrieves VMs in the specified namespace
	ListVirtualMachines(namespace string, opts metav1.ListOptions) ([]*VirtualMachine, error)
}

// GetVirtualMachine returns the VirtualMachine
func (c *Client) GetVirtualMachine(namespace, name string) (*VirtualMachine, error) {
	if err := c.initClient(); err != nil {
		return nil, err
	}
	vmRaw, err := c.client.Resource(vmResource).Namespace(namespace).Get(name, metav1.GetOptions{})
	if err != nil {
		return nil, err
	}
...
func (c *Client) ListVirtualMachines(
	...
	rawVMs, err := c.client.Resource(vmResource).Namespace(namespace).List(opts)
	if err != nil {
		return nil, err
	}
